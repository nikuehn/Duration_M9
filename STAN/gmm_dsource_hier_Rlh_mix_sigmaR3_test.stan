...
generated quantities{
  ...
  for(i in 1:NTEST) {
    ...
    real mu = log(logistic_hinge(R_test[i], R_b, c1 + r1 * R_b, r1, r2, delta_r));
    log_lik_test[i] = lognormal_lpdf(Y_test_exp[i] | mu, sqrt(square(sigma_path) + square(sigma_source)));
  }
